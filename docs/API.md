...
- `?dl=1` 强制 `attachment`（默认图片/音视频/文本等可预览类型是 `inline`）。
- 响应头带 `Access-Control-Allow-Origin: *`，可直接在网页里引用。
- 状态码：`200` / `206` / `304` / `404`（不存在、已删除）/ `410`（已过期）/
  `403`（来源被封禁，带 `Retry-After`）。

### `POST /api/upload`

| 位置 | 名称 | 说明 |
| --- | --- | --- |
| Header | `Authorization: Bearer <上传密钥>` | 也可用 `X-Assets-Key` 或 `?key=` |
...
状态码：`201`、`400`（缺少文件名/空 body/非法 hash）、`401`（密钥无效）、
`403`（来源被封禁）、`409`（hash 已被占用）、`413`（超过大小上限）、`429`（上传过于频繁）。
